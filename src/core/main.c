...
static int wait_for_ksu_status(void) {
    ...
    if (fd >= 0) {
      ...
      if (n > 0) {
        ...
        if (!strcmp(status, "ready")) {
          pr_success("root script reports ready\n");
          return 0;
        }
        if (!strncmp(status, "failed:", 7)) {
          pr_error("root script reports %s; see .ghostlock_root.log\n", status);
          return 1;
        }
      }
    }
  ...
  pr_error("root script did not report readiness; see .ghostlock_root.log\n");
  return 1;
}
...
static void write_root_script(void) {
  ...
  /* Goal: verify root, then restart zygote so the permissive SELinux state
   * takes effect for the freshly spawned framework processes. SELinux is NOT
   * restored to enforcing — the whole point is to keep it permissive. No
   * KernelSU/policy steps: this build is Path-B (no misc_fops redirect) and
   * the device has no KernelSU components installed, so those would only
   * stall the script. */
  const char *script =
    "#!/system/bin/sh\n"
    ...
    "diag '[*] selinux enforce='$(cat /sys/fs/selinux/enforce 2>/dev/null)' getenforce='$(getenforce 2>/dev/null)\n"
    "\n"
    "# ---- load kernelpatch.ko ----\n"
    "# Two load strategies, toggled by USE_KO_PATCH:\n"
    "#   0 (default): insmod the unmodified template with kln=<addr> param.\n"
    "#                Verified stable on this device.\n"
    "#   1          : binary-patch the KPLNSLOT slot in the .ko, then bare insmod.\n"
    "#                Suspected of causing the insmod panic; kept for A/B testing.\n"
    "USE_KO_PATCH=0\n"
    "KO_SRC=/data/local/tmp/android16-6.12_kernelpatch.ko\n"
    "KO_DST=/data/adb/ap/android16-6.12_kernelpatch.ko\n"
    "fail() { diag \"[!] abort: $1\"; report_status \"failed:$1\"; exit 1; }\n"
    "diag '=== [1] root identity ==='\n"
    "id\n"
    "diag '=== [2] relax kptr_restrict ==='\n"
    "echo 0 > /proc/sys/kernel/kptr_restrict 2>/dev/null; cat /proc/sys/kernel/kptr_restrict\n"
    "diag '=== [3] resolve kallsyms_lookup_name ==='\n"
    "grep kallsyms_lookup_name /proc/kallsyms\n"
    "KLN=$(grep ' T kallsyms_lookup_name' /proc/kallsyms | head -1 | awk '{print $1}')\n"
    "diag \"raw KLN=[$KLN]\"\n"
    "[ -n \"$KLN\" ] || fail 'kln-unresolved'\n"
    "KLN=0x$KLN\n"
    "diag \"KLN=[$KLN]\"\n"
    "[ \"$KLN\" != \"0x0\" ] && [ \"$KLN\" != \"0x0000000000000000\" ] || fail 'kln-zero'\n"
    "diag '=== [4] stage template -> '$KO_DST' ==='\n"
    "mkdir -p /data/adb/ap || fail 'mkdir-ap'\n"
    "cp \"$KO_SRC\" \"$KO_DST\" || fail 'cp-template'\n"
    "if [ \"$USE_KO_PATCH\" = \"1\" ]; then\n"
    "  diag '=== [5p] locate KPLNSLOT magic and patch addr slot ==='\n"
    "  _line=$(grep -abo KPLNSLOT \"$KO_DST\" | head -n 1) || true\n"
    "  _off=${_line%%:*}\n"
    "  [ -n \"${_off:-}\" ] || fail 'no-KPLNSLOT-slot'\n"
    "  OFF=$((_off + 8))\n"
    "  ESC=\"\"\n"
    "  i=0\n"
    "  while [ $i -lt 8 ]; do\n"
    ...
    "    i=$((i + 1))\n"
    "  done\n"
    "  printf '%b' \"$ESC\" | dd of=\"$KO_DST\" bs=1 seek=\"$OFF\" count=8 conv=notrunc 2>/dev/null\n"
    "  diag \"patched kln=$KLN at file offset 0x$(printf '%x' \"$OFF\")\"\n"
    "  # read-back verify: the 8 bytes at OFF must equal KLN little-endian\n"
    "  _rb=$(dd if=\"$KO_DST\" bs=1 skip=\"$OFF\" count=8 2>/dev/null | od -An -tx1 | tr -d ' \\n')\n"
    "  _exp=\"\"\n"
    "  j=0\n"
    "  while [ $j -lt 8 ]; do\n"
    "    _exp=\"$_exp$(printf '%02x' $(( (${KLN} >> (j * 8)) & 0xff )))\"\n"
    "    j=$((j + 1))\n"
    "  done\n"
    "  diag \"readback=[$_rb] expected=[$_exp]\"\n"
    "  [ \"$_rb\" = \"$_exp\" ] || fail 'patch-verify-mismatch'\n"
    "  diag '=== [6p] unload old kernelpatch ==='\n"
    "  rmmod kernelpatch 2>/dev/null; echo done\n"
    "  diag '=== [7p] insmod (patched, no kln=) ==='\n"
    "  insmod \"$KO_DST\" || fail 'insmod-patch'\n"
    "else\n"
    "  diag '=== [5k] unload old kernelpatch ==='\n"
    "  rmmod kernelpatch 2>/dev/null; echo done\n"
    "  diag '=== [6k] insmod (unmodified, kln='$KLN') ==='\n"
    "  insmod \"$KO_DST\" kln=$KLN || fail 'insmod-kln'\n"
    "fi\n"
    "diag \"insmod ok\"\n"
    "diag '=== [8] dmesg ==='\n"
    "dmesg | grep -i kernelpatch | tail -15\n"
